...
async function main() {
  try {
    ...
    const documents = [
      new Document({
        pageContent:
          "JavaScript adalah bahasa pemrograman tingkat tinggi yang dinamis dan ditafsirkan.",
        metadata: { source: "programming_guide", category: "javascript" },
      }),
      new Document({
        pageContent:
          "TypeScript adalah superset dari JavaScript yang menambahkan tipe statis.",
    ...
    const faissResultsWithScore =
      await loadedFaissStore.similaritySearchWithScore(query, 3);
    ...
    try {
      ...
      if (mmrResults) {
        ...
        console.log(
          "maxMarginalRelevanceSearch tidak tersedia untuk FaissStore",
        );
      }
    } catch (error) {
      console.log(
        "maxMarginalRelevanceSearch tidak didukung atau terjadi error:",
        error,
      );
    }
    ...
    const chromaStore = await Chroma.fromDocuments(documents, embeddings, {
      ...
      url: "http://127.0.0.1:8000", // URL Chroma server (jika running)
    });
    ...
    const filteredResults = await chromaStore.similaritySearch(
      "JavaScript",
      2,
      {
        category: "javascript",
    ...
    const newDoc = new Document({
      pageContent:
        "Vue.js adalah framework JavaScript progresif untuk membangun UI.",
    ...
    const newResults = await loadedFaissStore.similaritySearch(
      "Vue.js framework",
      1,
    );
    ...
    console.log(
      "Penghapusan dokumen tersedia di Chroma tapi tidak di FaissStore",
    );
